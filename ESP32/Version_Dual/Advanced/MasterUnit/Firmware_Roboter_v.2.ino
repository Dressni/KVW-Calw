...
void setup() {
  ...
  ledColor[0] = preferences.getInt("ledColorR", 255);
  ledColor[1] = preferences.getInt("ledColorG", 0);
  ledColor[2] = preferences.getInt("ledColorB", 0);
...
void udpThread(void *pvParameters) {
  boolean blockedDemo = false;
  unsigned long timeoutTimeMillis = 0;
  while (1) {
    ...
    if (packetSize)
    {
      ...
      if (recev.startsWith("SETMODE=auto")) {
        ...
        String answer = "RESPONSEMODE&operationMode=" + String(mode) + "&demoMode=" + String(demoMode) + "&spkMode=" + String(spkMode);
        ...
        } if (demoMode && !blockedDemo && (millis() - timeoutTimeMillis >= 2000)) {
          timeoutTimeMillis = millis();
          blockedDemo = true;
          state = 4;
          Beide();
          delay(1500);
          Keine();
          Serial.println("Demo-End: " + String(millis()));
          blockedDemo = false;
        }
      } else if (recev.startsWith("SETLEDCOLOR=")) {
        String dataArgs = recev.substring(12);
        int commaIndex = dataArgs.indexOf(';');
        int secondCommaIndex = dataArgs.indexOf(';', commaIndex + 1);
        ledColor[0] = dataArgs.substring(0, commaIndex).toInt();
        ledColor[1] = dataArgs.substring(commaIndex + 1, secondCommaIndex).toInt();
        ledColor[2] = dataArgs.substring(secondCommaIndex + 1).toInt();
        preferences.putInt("ledColorR", ledColor[0]);
        preferences.putInt("ledColorG", ledColor[1]);
        preferences.putInt("ledColorB", ledColor[2]);
        Serial.println("Neue LED-Farbe: " + String(String(ledColor[0]) + ";" + String(ledColor[1]) + ";" + String(ledColor[2])));
      } else if (recev.startsWith("REQUESTLEDCOLOR?")) {
        Serial.println("LED-Farbe: " + String(String(ledColor[0]) + ";" + String(ledColor[1]) + ";" + String(ledColor[2])));
        Udp.beginPacket(Udp.remoteIP(), Udp.remotePort());
        String toPrint = String(String("RESPONSECOLOR&") + String(String(ledColor[0]) + ";" + String(ledColor[1]) + ";" + String(ledColor[2])));
        Udp.print(toPrint);
        Udp.endPacket();
        delay(25);
        byte ct = 0;
      ...
      } else if (recev.startsWith("SETDEMOMODE=")) {
        String argument = recev.substring(12);
        if (argument.equals("true")) {
          demoMode = true;
        } else {
          demoMode = false;
        }
        Serial.println("Demo-Mode: " + String(demoMode));
      } else if (recev.startsWith("SETSPKMODE=")) {
        String argument = recev.substring(11);
        if (argument.equals("true")) {
          spkMode = true;
        } else {
          spkMode = false;
        }
        Serial.println("SPK: " + String(spkMode));
      }
...
void loop() {
  ...
  oled.drawStringMaxWidth(0, 0, 128, String("Rx: " + String(oldrx)));
